...
export function normalizeSalary(salaryText: string): SalaryInfo {
  ...
  if (salaryText.toLowerCase().includes('/month') || salaryText.toLowerCase().includes('monthly')) period = 'monthly';
  else if (salaryText.toLowerCase().includes('/hour') || salaryText.toLowerCase().includes('hourly')) period = 'hourly';

  return {
    min: parsedNumbers.length > 0 ? Math.min(...parsedNumbers) : null,
    max: parsedNumbers.length > 0 ? Math.max(...parsedNumbers) : null,
    currency, period, raw: salaryText
  };
}

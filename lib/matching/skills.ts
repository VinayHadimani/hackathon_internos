...
function normalize(skill: string): string {
  return skill.toLowerCase().trim().replace(/[^a-z0-9/#\s]/g, '').replace(/\s+/g, ' ').trim();
}
...
export function findMissingSkills(userSkills: string[], requiredSkills: string[]): string[] {
  if (!requiredSkills || requiredSkills.length === 0) return [];
  if (!userSkills || userSkills.length === 0) return [...requiredSkills];
  return requiredSkills.filter(req => !userSkills.some(user => isMatch(user, req)));
}

export function findMatchedSkills(userSkills: string[], requiredSkills: string[]): string[] {
  if (!requiredSkills || requiredSkills.length === 0) return [];
  if (!userSkills || userSkills.length === 0) return [];
  return requiredSkills.filter(req => userSkills.some(user => isMatch(user, req)));
}

export function getMatchGrade(score: number): { grade: string; color: string; label: string } {
...
export function rankInternships(userSkills: string[], internships: Array<{ id: string; skills: string[] }>): Array<{ id: string; score: number }> {
  return internships.map(i => ({ id: i.id, score: calculateMatchScore(userSkills, i.skills) })).sort((a, b) => b.score - a.score);
}
